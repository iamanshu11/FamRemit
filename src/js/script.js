...
const swiper = new Swiper('.swiper-container', {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    let swiper = new Swiper(".swiper-container-custom", {
        ...
        on: {
            slideChangeTransitionEnd: function () {
                document.querySelectorAll('.swiper-slide-custom').forEach((slide, index) => {
                    ...
                    if (slide.classList.contains('swiper-slide-active')) {
                        slide.classList.add('swiper-slide-active-custom'); // Only active moves left
                    } 
                    // Two slides above active should remain visible
                    else if (
                        index === swiper.activeIndex - 1 ||
                        index === swiper.activeIndex - 2 ||
                        (swiper.activeIndex === 0 && index === swiper.slides.length - 1)
                    ) {
                        slide.classList.add('swiper-slide-above');
                    } 
                    // One slide below active should remain visible
                    else if (index === swiper.activeIndex + 1 || index === 0) {
                        slide.classList.add('swiper-slide-below');
                    } 
                    // All other slides fade out
                    else {
    ...
    // Initial class setup
    document.querySelector('.swiper-slide-active').classList.add('swiper-slide-active-custom');
});
